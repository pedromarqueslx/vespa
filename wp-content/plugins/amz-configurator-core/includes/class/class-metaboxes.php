	...
	if (!class_exists('Amazee_Metabox')) {
		...
		class Amazee_Metabox {
			...
			public function metabox_register_style( $hook_suffix ) {
				...
				$amz_post_types = array( 'post', 'page', 'product' );

				if( ( 'post.php' == $hook_suffix || 'post-new.php' == $hook_suffix ) && in_array( $post_type, $amz_post_types ) ) {

					//Load css
					wp_enqueue_style( 'metabox-css', CONFIGURATOR_CORE_PLUGIN_URL .'/assets/css/metabox.css', array(), $this->version );

					//Scripts
					wp_enqueue_style( 'wp-color-picker' );

					wp_enqueue_script( 'metabox_admin_js', CONFIGURATOR_CORE_PLUGIN_URL .'/assets/js/plugin.js', array( 'jquery', 'wp-color-picker', 'jquery-ui-datepicker' ), $this->version);

					wp_enqueue_script( 'metabox_plugin_js', CONFIGURATOR_CORE_PLUGIN_URL .'/assets/js/script.js', array( 'jquery', 'wp-color-picker', 'jquery-ui-datepicker' ), $this->version);

					wp_enqueue_script( 'metabox_media_manager', CONFIGURATOR_CORE_PLUGIN_URL .'/assets/js/media-upload.js', array( 'jquery','jquery-ui-sortable' ), $this->version);
									...
									$this->fields_html .= '<ul class="pix-image-select float-clear">';
										...
										foreach ( $field['options'] as $key => $opt ) {
											...
											$this->fields_html .= '<li>';
												...
												$this->fields_html .= '<a href="#"><img src="'. CONFIGURATOR_CORE_PLUGIN_URL . '/assets/img/' . esc_attr( $opt ) . '" alt="" /><i class="icon ct-check"></i></a>';
											$this->fields_html .= '</li>';
